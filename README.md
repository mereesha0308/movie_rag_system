...
## Video walkthrough script (~2 min Loom)

1. **Intro (10s):** "This is a mini RAG system that answers questions about movie
   plots using a single LangGraph agent. It connects data → embeddings → retrieval →
   LLM → structured JSON."
2. **Pipeline walkthrough (45s):** open `src/agent.py`; explain the agent loop —
   it calls the `retrieve_movie_plots` tool against a Chroma store of chunked
   plot embeddings, then answers grounded on the retrieved context (or asks a
   clarifying question if the query is vague).
3. **Demo (45s):** run `python main.py build-index` (show "chunks stored"),
   then `python main.py query "..."` and show the JSON with `answer`, `contexts`,
   `reasoning`. Also show a vague query returning a `clarifying_question`.
4. **Wrap-up (20s):** mention `config/config.yaml` for tuning and the tests
   (`python -m pytest tests/ -q`).
...
git init
git add .
git commit -m "Mini RAG system for movie plots"
git branch -M main
git remote add origin git@github.com:<username>/<repo>.git
git push -u origin main
```

Then set `GROQ_API_KEY` as a repo secret if you demo in CI.
